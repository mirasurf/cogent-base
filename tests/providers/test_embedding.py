...
class TestIntegrationLiteLLMEmbedding:
    ...
    async def test_ollama_object_embedding(self, mock_config, test_texts):
        ...
        with patch('cogent.providers.embedding.litellm_embedding.get_config') as mock_get_config:
...
class TestLiteLLMEmbeddingUnit:
        ...
        def _make_response(num_embeddings=3, dim=765):
            class MockResponse:
                def __init__(self, num_embeddings, dim):
                    self.data = [
                        {"embedding": [float(i % 10) for i in range(dim)]} for _ in range(num_embeddings)
                    ]
    ...
    async def test_mocked_litellm_embedding(self, mock_config, mock_litellm_response):
        ...
        with patch('cogent.providers.embedding.litellm_embedding.get_config') as mock_get_config, \
             patch('cogent.providers.embedding.litellm_embedding.litellm.aembedding', new_callable=AsyncMock) as mock_litellm:
    ...
    async def test_mocked_ollama_embedding(self, mock_config, mock_litellm_response):
        ...
        with patch('cogent.providers.embedding.litellm_embedding.get_config') as mock_get_config, \
             patch('cogent.providers.embedding.litellm_embedding.ollama.AsyncClient') as mock_ollama_client:
    ...
    async def test_mocked_error_handling(self, mock_config):
        ...
        with patch('cogent.providers.embedding.litellm_embedding.get_config') as mock_get_config, \
             patch('cogent.providers.embedding.litellm_embedding.litellm.aembedding') as mock_litellm:
    ...
    async def test_dimension_validation_warning(self, mock_config, mock_litellm_response):
        ...
        with patch('cogent.providers.embedding.litellm_embedding.get_config') as mock_get_config, \
             patch('cogent.providers.embedding.litellm_embedding.litellm.aembedding', new_callable=AsyncMock) as mock_litellm, \
             patch('cogent.providers.embedding.litellm_embedding.logger') as mock_logger:
