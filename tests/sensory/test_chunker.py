...
from cogent.base.sensory.chunker.standard_chunker import StandardChunker, RecursiveCharacterTextSplitter
...
class TestContextualChunker(unittest.TestCase):
    ...
    @pytest.mark.unit
    @patch('cogent.sensory.chunker.contextual_chunker.get_config')
    def test_init_success(self, mock_get_config):
        ...
        self.assertEqual(chunker.model_config, {
            "model": "test_model",
            "api_base": "http://localhost:11434",
            "api_key": "test_key"
        })
    ...
    @pytest.mark.unit
    @patch('cogent.sensory.chunker.contextual_chunker.get_config')
    @patch('cogent.sensory.chunker.contextual_chunker.LiteLLMCompletionModel')
    @pytest.mark.asyncio
