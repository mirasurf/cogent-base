...
class TestCogentParser:
    ...
    def test_init_contextual_chunking(self, mock_get_config):
        """Test parser initialization with contextual chunking."""
        # Patch config to include 'ollama_qwen_vision' in registered_models and as contextual_chunking_model
        mock_get_config.return_value = type(
            "MockConfig",
            (),
            {
                "llm": type(
                    "MockLLM", (), {"registered_models": {"ollama_qwen_vision": {"model_name": "qwen2.5vl:latest"}}}
                )(),
                "sensory": type("MockSensory", (), {"contextual_chunking_model": "ollama_qwen_vision"})(),
            },
        )()
        parser = CogentParser(
            chunk_size=500,
    ...
    async def test_split_text_contextual_chunking(self, mock_contextual_chunker_class, mock_get_config):
        """Test text splitting with contextual chunking."""
        # Patch config to include 'ollama_qwen_vision' in registered_models
        mock_get_config.return_value = type(
            "MockConfig",
            (),
            {
